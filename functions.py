...
__all__=['proj_l1ball','centroids','class2indicator','nb_Genes','select_feature_w','compute_accuracy',
            'predict_L1','predict_FISTA','normest','merge_topGene_norm','merge_topGene_norm_acc',
            'compare_2topGenes','heatmap_classification','heatmap_normW','drop_cells',
            'FISTA_Primal','primal_dual_L1N','primal_dual_Nuclear',
            'basic_run_eta','basic_run_tabeta',
            'run_FISTA_eta','run_primal_dual_L1N_eta','run_primal_dual_Nuclear_eta',
            'run_FISTA_tabeta','run_primal_dual_L1N_tabeta','run_primal_dual_Nuclear_tabEtastar']
...
| heatmap_classification             | Ytest,YR,clusternames,rotate,draw_fig                                        | Heatmap_matrix            | Function |
| heatmap_normW                      | normW, clusternames,nbr_L,rotate,draw_fig                                    | Heatmap_matrix            | Function |
...
def proj_l1ball(y,eta):    
    ...
    if y.ndim >1:
        y = np.reshape(y,(-1,))
    return np.maximum( np.absolute(y)-np.amax([np.amax( (np.cumsum(np.sort(np.absolute(y),axis = 0)[::-1],axis = 0) - eta)\
         /(np.arange(y.shape[0])+1) ),0]), 0)*np.sign(y)
...
def basic_run_eta_compare(func_algo, func_predict,
    ...
    for i,(train_ind, test_ind) in enumerate(kf.split(YR)):
        ...
        for j in range(numalg):
            ...
            accuracy_train_comp[i][j] = accuracy_score(Ypred_train.astype('int64'),Ytrain)
    ...
    alglist1.append('pd')
    df_timeElapsed = pd.DataFrame(timeElapsedMatrix, index=ind_df_comp, columns=alglist1)
    ...
    return mu_mean, nbm, accG, loss, W_mean, timeElapsed, \
            df_topGenes, df_normW, df_topG_normW, \
            df_topGenes_mean, df_normW_mean, df_topG_normW_mean,\
            df_acctest,df_comp,df_timeElapsed,w_all
...
def run_FISTA_eta(X,YR,k,genenames=None,clusternames=None,
    ...
    return basic_run_eta(FISTA_Primal,predict_FISTA,
                                 ...
                                 beta=beta,delta=delta,eta = eta, gamma=gamma,  showres=showres,                    
...
def run_primal_dual_L1N_eta(X,YR,k,genenames=None,clusternames=None,
    ...
    return basic_run_eta(primal_dual_L1N,predict_L1,
                         ...
                         outputPath=outputPath)
def run_primal_dual_L1N_eta_compare(X,YR,k,alglist,genenames=None,clusternames=None,
                        niter=30,
                        rho = 1,
                        tau = 1.5,
                        beta = 0.25,
                        ...
                        keepfig = False,
                        outputPath='../results/'):    
    return basic_run_eta_compare(primal_dual_L1N,predict_L1,
                         X,YR,k,alglist, genenames,clusternames,niter=niter,
                         beta=beta,delta=delta,eta=eta,rho=rho,tau=tau,
                         nfold= nfold,rng=random_seed,
                         showres = showres,saveres=saveres,keepfig=keepfig,
                         outputPath=outputPath)
